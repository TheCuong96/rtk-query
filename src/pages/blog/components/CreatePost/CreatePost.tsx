...
export default function CreatePost() {
  ...
  const { data } = useGetPostQuery(selectItemEdit, { skip: !selectItemEdit }) // { skip: !selectItemEdit } để nó bắt sự kiện khi có data trong selectItemEdit thì nó mới chạy, còn không thì nó không chạy thằng này, để tránh phải chạy 1 api vô nghĩa khi ban đầu render ra
  useEffect(() => {
    if (data) {
      setFormData(data)
    }
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (selectItemEdit) {
      await updatePost({ body: formData as Post, id: selectItemEdit })
    } else {
